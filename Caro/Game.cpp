...
void undoLastMove() {
    ...
    if (getCheckAtXY(x, y) == -1) _b->CountX--;
    else if (getCheckAtXY(x, y) == 1) _b->CountY--;
    g->moves.pop_back();
    undo(x, y);
}
...
void goiY() {
    int x = getXatEnter();
    int y = getYatEnter();
    int check = getCheckAtXY(x, y);
    if(check == 0){
        addMove(x, y); undoLastMove(); // Xóa biểu tượng tại vị trí (x, y)
    }
